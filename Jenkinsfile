pipeline {
    ...
    environment {
        ...
        REPO_URL = 'https://github.com/Nest-Microservices-Product/shopping-cart-service'
        SSH_CRED_ID = 'ssh-key-ec2'
        SSH_CRED_ID_DIEGO = 'ssh-key-ec2-diego'
        EC2_USER = 'ubuntu'
        REMOTE_PATH = '/home/ubuntu/shopping-cart-service'
        K8S_REMOTE_PATH = '/home/ubuntu/nest-microservices/k8s/store-ms/templates/shopping-cart-ms/'
        IMAGE_NAME = 'fernandoflores07081/shopping-cart-ms-prod'
    }
        ...
        stage('Deploy to GKE - Production') {
            ...
            steps {
                script {
                    ...
                    sh "kubectl set image deployment/shopping-cart-ms shopping-cart-ms-prod=${IMAGE_NAME}:${env.DOCKER_TAG} --namespace=default"

                    // Verificar que el despliegue se complete exitosamente
                    sh "kubectl rollout status deployment/shopping-cart-ms --namespace=default"
                }
            }
        }
